...
def load_menu(ln=CFG_SITE_LANG):
    ...
    <li class="hassubmenu">
        <a href="#">%(Rule_management)s</a>
        <ul class="submenu">
            <li><a href="%(url)s/admin2/bibcirculation/loan_rules">%(Loan_rules)s</a></li>
            <li><a href="%(url)s/admin2/bibcirculation/patron_types">%(Patron_types)s</a></li>
        </ul>
    </li>
    ...
    """ % {'url': CFG_SITE_URL, 'ILL': _("ILL"),
           ...
           'Rule_management': _("Rule management"),
           'Loan_rules': _("Loan rules"),
           'Patron_types': _("Patron types"),
           'Help': _("Help"),
...
class Template:
    ...
    def tmpl_holdings_information(self, recid, req, holdings_info, bibformat_element=False,
        ...
        for (barcode, library, collection, call_no, location, description,
            ...
            if bibcirc_user:
                out += """
                    <td>%s</td>
                    """ % render_loan_period(loan_period)
            out += """
                    <td>%s</td>
                    <td>%s</td>
    ...
    def tmpl_index(self, infos=None, ln=CFG_SITE_LANG):
        """
...
### FIXME: If one result -> go ahead ###
        ...
        if loan_info:
            ...
            <link rel="stylesheet" type="text/css" href="/img/jquery.simple-dtpicker.css" />
              ...
              <table class="tablesorter" border="0" cellpadding="0" cellspacing="1">
                ...
                <tr>
                  <th width="70">%s</th>
                  <td><form action="/admin2/bibcirculation/change_due_date_step2" method="get">
            ...
            if " " not in actual_due_date:
                out += """$("#datetime_picker1").appendDtpicker({'locale': '%s', "current": "%s", 'firstDayOfWeek': 1, "dateFormat": "YYYY-MM-DD hh:mm", "closeOnSelected": true, "dateOnly": true});""" % (ln, actual_due_date)
            else:
                out += """$("#datetime_picker1").appendDtpicker({'locale': '%s', "current": "%s", 'firstDayOfWeek': 1, "dateFormat": "YYYY-MM-DD hh:mm", "closeOnSelected": true});""" % (ln, actual_due_date)
    ...
    def tmpl_change_due_date_step1(self, loan_details, loan_id, borrower_id, loan_period,
        ...
        if loan_period and loan_period['type'] != 'hours':
            out += """$("#datetime_picker1").appendDtpicker({'locale': '%s', 'firstDayOfWeek': 1, "dateFormat": "YYYY-MM-DD hh:mm", "closeOnSelected": true, "dateOnly": true});""" % ln
        else:
            out += """$("#datetime_picker1").appendDtpicker({'locale': '%s', 'firstDayOfWeek': 1, "dateFormat": "YYYY-MM-DD hh:mm", "closeOnSelected": true});""" % ln
...
""" % (_("The due date has been updated. New due date: %s" % (new_due_date)),
    ...
    def tmpl_all_loans(self, result, infos, ln=CFG_SITE_LANG,  libraries=()):
        """
        @param ln: language of the page
        """
...
###### END STATUSES ######
    ...
    def tmpl_rules_selection(self, rules_selections, rules, itemtypes, patrontypes, infos, ln=CFG_SITE_LANG):
        """
        @param results: all current loan rules
        @type results: tuple
        ...
        out += """
            <br />
            <table id="table_rules_selection" class="tablesorter"
                   border="0" cellpadding="0" cellspacing="1">
               <thead>
                    <tr>
                       <th>%s</th>
        ...
        for (id, rule, itemtype, patrontype, location, active) in rules_selections:
            ...
            out += """
                    <tr>
                        ...
                        <td>%s</td>
                        <td>%s</td>
                        <td><input type="button" value="%s" class="bibcircbutton" onClick="window.location='rules_selection?toggle=%s'"></td>
        ...
        out += """
                    <h3 style="margin-bottom: 5px">%s</h3>
                    <form name="add_rule" action="%s/admin2/bibcirculation/rules_selection" method="get">
                    <table id="new_rule" class="tablesorter">
                       <thead>
                            <tr>
                               <th>%s</th>
        ...
        for id, patrontype in patrontypes:
            out += """
                                    <option value="%s">%s</option>
                   """ % (id, patrontype)
        ...
        out += """
                                 ...
                                 <td><select name="active"><option>Y</option><option>N</option></select></td>
                                 <td><input type="submit" value="%s" class="bibcircbutton"></td>
                             </tr>
                         </tbody>
                     </table>
    ...
    def tmpl_location_exceptions(self, result, infos, ln=CFG_SITE_LANG):
        ...
        for id, code, location, library in locations:
                out += """<option value="%s">%s - %s</option>""" % (id, code, location)
