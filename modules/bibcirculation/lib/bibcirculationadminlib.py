...
def place_new_request_step3(req, barcode, recid, user_info,
    ...
    if not loan_rule:
        infos.append(_('%(x_strong_tag_open)sError:%(x_strong_tag_close)s This patron does not have permission to loan this item.' % {'x_strong_tag_open': '<strong>', 'x_strong_tag_close': '</strong>'}))
        body = bc_templates.tmpl_place_new_request_step1(barcode=barcode,
                                                         recid=recid,
                                                         user_info=user_info,
                                                         infos=infos,
                                                         ln=ln)
...
def all_loans(req, msg=None, ln=CFG_SITE_LANG, library=(), loans_per_page=0, nb_page=0):
    """
    Display all loans.
...
def all_loans_data(req, msg=None, ln=CFG_SITE_LANG, library=(), loans_per_page=0, nb_page=0):
    """
    Display all loans.
...
def patron_types(req, name, delete, ln=CFG_SITE_LANG):
    ...
    if type(delete) == int:
        try:
            ...
            infos.append(_("%(x_strong_tag_open)sError:%(x_strong_tag_close)s Patron type in use."
                           % {'x_strong_tag_open': '<strong>',
                              'x_strong_tag_close': '</strong>'
                              }))
        except DatabaseError:
            infos.append(_("%(x_strong_tag_open)sError:%(x_strong_tag_close)s The patron type ID you are trying to delete does not exist."
                           % {'x_strong_tag_open': '<strong>',
                              'x_strong_tag_close': '</strong>'
                              }))
    ...
    result = db.get_patron_types()
    body = bc_templates.tmpl_patron_types(result=result, infos=infos, ln=ln)

    navtrail_previous_links = '<a class="navtrail" ' \
                              'href="%s/help/admin">Admin Area' \
                              '</a>' % (CFG_SITE_SECURE_URL,)

    return page(title=_("Patron types"),
                uid=id_user,
                req=req,
                body=body, language=ln,
                navtrail=navtrail_previous_links,
